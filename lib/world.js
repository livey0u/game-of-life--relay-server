...
class World extends EventEmitter {
	...
	evolve() {
		...
		this.emit(constants.EVOLUTION_EVENT, {layout: nextGenerationLayout, evolvedAt: this.evolvedAt});
	...
	evolveCell(cell) {
		...
		if(cell.state === DEAD) {
			if(liveNeighboursCount === 3) {
				...
				nextGenCell.color = this.getColorAverage(liveNeighbours);
			}
		}
		else {
	...
	getColorAverage(cells) {

		let colors = cells.map(function fn(cell) {
			return cell.color;
		});
	...
	setCell(data) {
		...
		if(data.lastEvolvedAt !== this.evolvedAt) {
			throw new Error('EVOLVED_ALREADY', 'Retry');
		}
		...
		this.emit(constants.CELL_UPDATED_EVENT, {layout: this.layout, cell: cell, evolvedAt: this.evolvedAt});
